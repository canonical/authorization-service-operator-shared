...
1. **Module `authorization_service_operator_shared.info`**:
   ...
   - `AuthorizationServiceInfoProvider`: Juju provider wrapper emitting `AuthorizationServiceInfoRelationReadyEvent` when a relation joins and providing `publish_info(...)` to create/update Juju secrets (`db-password`), grant relation access, and write versions and connection details into the application databag when leader.
   - `AuthorizationServiceInfoRequirer`: Juju requirer wrapper emitting `AuthorizationServiceInfoBrokenEvent` and providing `get_info()` to read databag details and retrieve database secrets.
...
4. **Unit Tests**: Implement complete unit tests in `tests/unit/test_info.py`.
