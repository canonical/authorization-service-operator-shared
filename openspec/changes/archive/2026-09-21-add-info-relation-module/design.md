...
- Handles `relation_broken` events on `authorization-service-info` on leader units:
  - Calls `secret.revoke(event.relation)` to revoke secret access for the departing relation.
  - Checks if any active relations remain (`remaining = [r for r in relations if r.id != event.relation.id]`).
  - If no active relations remain, calls `secret.remove_all_revisions()` to destroy the secret from Juju secret storage.
...
- `get_info() -> AuthorizationServiceInfo | None`: Reads relation app databag, fetches secret content via `db_secret_id` using `secret.get_content(refresh=True)` to ensure latest secret payload revisions are retrieved, and returns an `AuthorizationServiceInfo` instance (or `None` if relation is absent or missing data).
