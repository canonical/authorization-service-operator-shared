...
def test_migrate_status_returns_true_when_pending_in_stderr() -> None:
    dsn = "postgresql://user:pass@host:5432/db"
    cli, _ = _make_cli(exec_result=("", "2 migrations pending\n"))
    assert cli.migrate_status(dsn) is True
...
def test_migrate_status_raises_on_exec_error() -> None:
    dsn = "postgresql://user:pass@host:5432/db"
    container = MagicMock()
    err = ops.pebble.ExecError(["/usr/bin/app", "migrate", "status"], 1, "", "db connection failed")
    container.exec.side_effect = err
