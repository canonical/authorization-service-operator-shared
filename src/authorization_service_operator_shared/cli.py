...
class CommandLine:
    ...
    def migrate_status(self, dsn: str) -> bool:
        ...
        stdout, stderr = self._run_cmd([self.app_binary, "migrate", "status", "--dsn", dsn], config)
        output = f"{stdout}\n{stderr}"
        return "pending" in output.lower()
    ...
    def create_openfga_model(self, store_id: str, env: dict[str, str] | None = None) -> str:
        ...
        if model_id := exec_env.get("OPENFGA_AUTHORIZATION_MODEL_ID"):
            cmd.extend(["--fga-model-id", model_id])
        ...
        raise CreateFgaModelError(f"write-model output did not contain a model_id. stdout: {stdout}, stderr: {stderr}")
